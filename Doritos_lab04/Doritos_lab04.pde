...
void setup() {
  ...
  for (int i = 0; i < boardSize; i++) {
    for (int j = 0; j < boardSize; j++) {
      ...
      matrixButton = cp5.addButton("matrixButton")
        .setCaptionLabel(i + " , " + j)
        .setValue(1)
        .setPosition(x, y)
...
public void Score() {
  score.setColor(buttonPressedColor);
}

public void MatrixButton() {
  matrixButton.setColor(buttonPressedColor);
}
